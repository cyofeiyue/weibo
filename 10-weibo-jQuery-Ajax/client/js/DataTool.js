...
(function () {
    ...
    DataTool.saveItem = function (id, content, successFunc) {
        ...
        // $.get('../server/weibo-server.php', 'act=save&id='+id+'&content='+content, function () {
        //     successFunc();
        // });
    ...
    DataTool.getItemsWithPageAndSize = function (page, size, successFunc) {
        AjaxTool.get('../server/weibo-server.php', {
            ...
            if (xhr.responseText.length >= 3) {
                itemArray = JSON.parse(xhr.responseText);
            }
            successFunc(itemArray);
        }, function () {
            console.log('发布失败');
    ...
    // 根据id, 删除微博
    DataTool.deleteItemWithID = function (id, successFunc) {
